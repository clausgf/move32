...
void updateServoMode(uint8_t index) {
    if (index < SERVOS_MAX) {
        ...
        bool isFbForce_ChannelValid = isFbForce_ChannelIndexValid && (rxRead(fbForceChannel) > 0);

        bool isFbForce_FunctionActive = (fbForceChannel < RX_MAX_CHANNEL) && (fbForcePulseLenUs != 0);

        bool isFbForce_Predicate =
                ( fbForcePredicate == 0 && rxRead(fbForceChannel) < fbForcePulseLenUs ) ||
                ( fbForcePredicate == 1 && rxRead(fbForceChannel) > fbForcePulseLenUs );
        ...
        bool isFb_ChannelValid = isFb_ChannelIndexValid && (rxRead(fbRxChannel) > 0);
        ...
        if (isHost) {
            ...
            uint16_t rxPulseLenMs = rxRead(fbRxChannel);
            servoWrite(index, rxPulseLenMs);
        } else {
...
void cliProcess(char *cmdLine) {
    if (cmdLine[0] == '#' || cmdLine[0] == '\0') {
        ...
        if (servoIndex < SERVOS_MAX && pulseLenUs <= 3000) {
            servoSetFailsafe(servoIndex, pulseLenUs);
        ...
        for (uint8_t servoIndex=0; servoIndex<SERVOS_MAX; servoIndex++) {
            printf("failsafe %d %d\n", (int)servoIndex, (int)servoGetFailsafe(servoIndex));
        }
...
int main(void) {
    ...
    // configure the board including clock, leds and serial
    boardInit();
    //printf("\nSystemCoreClock = %lu\n", SystemCoreClock);
    ...
    while (1) {
        ...
        if ( (autoTransmitRxIntervalMs == 1 && wasLastRxChannelUpdated) ||
             (autoTransmitRxIntervalMs > 1 && isAutoTransmitTimerExpired ) ) {
                    transmitRx();
        ...
        // led0 (red) output
        if (failsafe) {
            led0Set(GPIO_PIN_RESET); // this switches the led on
        } else if (fallback) {
        ...
        // led1 (green) output
        if (HAL_GetTick() % 500 < 250) {
            led1Set(GPIO_PIN_RESET); // this switches the led on
        } else {
            led1Set(GPIO_PIN_SET); // this switches the led off
        }
    }
